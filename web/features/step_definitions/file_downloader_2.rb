...
Quando("eu deleto o arquivo") do
  expect(File).to exist(File.join(@downloads_directory, "logo.png"))

  # Deletando o arquivo baixado para nao poluir meu PC

  file1_to_delete = File.expand_path(File.join(@downloads_directory, "logo.png"))
...
Entao("o arquivo nao deve mais estar na pasta") do
  ...
  expect(File).not_to exist(File.join(@downloads_directory, "logo.png"))
end
